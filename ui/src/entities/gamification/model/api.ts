...
export const gamificationApi = {
  getMyAchievements: async (
    ...
    const { data } = await http.get<GetMyAchievementsGatewayResponse>('/api/users/me/achievements', params);
    return data;
  },

  getMyLevel: async (): Promise<GetLevelGatewayResponse> => {
    const { data } = await http.get<GetLevelGatewayResponse>('/api/users/me/level');
    return data;
  },

  getUserLevel: async (userId: string): Promise<GetLevelGatewayResponse> => {
    const { data } = await http.get<GetLevelGatewayResponse>(`/api/users/${userId}/level`);
    return data;
  },
  ...
  getUserAchievements: async (
    ...
    const { data } = await http.get<GetUserAchievementsGatewayResponse>(`/api/users/${userId}/achievements`, params);
    return data;
  },
};
