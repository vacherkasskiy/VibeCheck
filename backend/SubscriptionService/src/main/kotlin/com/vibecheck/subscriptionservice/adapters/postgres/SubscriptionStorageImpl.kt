...
class SubscriptionStorageImpl(
    ...
    override fun findAuthorIdsBySubscriberId(subscriberId: UUID): List<UUID> =
        subscriptionRepository.findAuthorIdsBySubscriberId(subscriberId)

    override fun findSubscriberIdsByAuthorId(authorId: UUID): List<UUID> =
        subscriptionRepository.findSubscriberIdsByAuthorId(authorId)

    override fun countSubscribersByAuthorId(): Map<UUID, Long> =
        subscriptionRepository.countSubscribersByAuthorId()
            .associate { it.getAuthorId() to it.getSubscribersCount() }
}
