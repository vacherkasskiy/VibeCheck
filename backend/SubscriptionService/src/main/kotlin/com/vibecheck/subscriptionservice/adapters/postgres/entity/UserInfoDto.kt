...
fun UserInfoDto.toDomain(): UserInfo =
    when (this) {
        is ReviewLikedInfoDto -> ReviewLikedInfo(reviewId, reviewAuthorId, companyId, companyName)
        is AchievementGrantedInfoDto -> AchievementGrantedInfo(achievementId, displayName)
        is ReviewWrittenInfoDto -> ReviewWrittenInfo(reviewId, companyId, companyName)
        is UserFollowedInfoDto -> UserFollowedInfo(userId, displayName)
        is UserLevelUpInfoDto -> UserLevelUpInfo(newLevel)
    }
